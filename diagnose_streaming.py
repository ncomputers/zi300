...
class StreamingDiagnostic:
    ...
    def test_redis_connection(self) -> None:
        ...
        with self.test_section("Redis Connection"):
            ...
            if cameras_data:
                ...
                for cam in cameras:
                    print(
                        f"     - Camera {cam.get('id')}: {cam.get('name')} ({cam.get('url', 'No URL')})"
                    )
            else:
    ...
    def test_camera_test_endpoint(self, url: str) -> None:
        ...
        with self.test_section(f"Camera Test Endpoint ({mask_credentials(url)})"):
            test_data = {"url": url, "transport": "tcp", "timeout": 10}

            response = requests.post(f"{self.base_url}/cameras/test", json=test_data, timeout=20)

            if response.status_code == 200:
                content_type = response.headers.get("content-type", "")
                if "image/jpeg" in content_type:
                    print(
                        f"   ✅ Test endpoint returned JPEG image ({len(response.content)} bytes)"
                    )
                else:
    ...
    def test_rtsp_connector(self, url: str, cam_id: int) -> RtspConnector:
        ...
        with self.test_section(f"RTSP Connector Test (Camera {cam_id})"):
            ...
            print("   ✓ RtspConnector created")
            print(f"   Initial state: {connector.state}")

            # Start connector
            connector.start()
            print("   ✓ RtspConnector started")
            ...
            while time.time() - start_time < timeout:
                ...
                print(
                    f"   Status: {stats['state']}, FPS: {stats['fps_in']}, Error: {stats.get('last_error', 'None')}"
                )
            ...
            if not frame_received:
                stats = connector.stats()
                raise Exception(
                    f"No frames received in {timeout}s. State: {stats['state']}, Error: {stats.get('last_error', 'Unknown')}"
                )
    ...
    def test_api_streaming_endpoints(self, cam_id: int) -> None:
        ...
        with self.test_section(f"API Streaming Endpoints (Camera {cam_id})"):
            ...
            if response.status_code == 200:
                stats = response.json()
                print(
                    f"   ✓ Camera stats: preview={stats.get('preview')}, online={stats.get('online')}"
                )
            ...
            try:
                response = requests.get(
                    f"{self.base_url}/api/cameras/{cam_id}/mjpeg", timeout=10, stream=True
                )
                ...
                if response.status_code == 200:
                    ...
                    if "multipart/x-mixed-replace" in content_type:
                        ...
                        if chunk_count > 0:
                            print(
                                f"   ✅ MJPEG stream is producing data ({chunk_count} chunks received)"
                            )
                        else:
    ...
    def _generate_recommendations(self) -> None:
        ...
        failed_tests = [
            name for name, result in self.test_results.items() if result["status"] == "FAILED"
        ]
    ...
    def run_full_diagnostic(
        self, camera_id: Optional[int] = None, camera_url: Optional[str] = None
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:8000", help="Base URL of the application"
    )
