import queue
from pathlib import Path
from types import SimpleNamespace

import fakeredis
import numpy as np

import modules.tracker.manager as manager_mod
from modules.tracker import InferWorker, PersonTracker, PostProcessWorker


def _make_tracker(tmp_path):
    r = fakeredis.FakeRedis()
    tracker = PersonTracker.__new__(PersonTracker)
    tracker.cam_id = 1
    tracker.line_orientation = "vertical"
    tracker.line_ratio = 0.5
    tracker.reverse = False
    tracker.groups = ["person"]
    tracker.in_counts = {}
    tracker.out_counts = {}
    tracker.tracks = {}
    tracker.track_states = {}
    tracker.track_state_ttl = 120.0
    tracker.frame_queue = queue.Queue()
    tracker.det_queue = queue.Queue()
    tracker.out_queue = queue.Queue()
    tracker.running = False
    tracker.viewers = 0
    tracker.snap_dir = Path(tmp_path)
    tracker.redis = r
    tracker.ppe_classes = []
    tracker.cfg = {}
    tracker.debug_stats = {}
    tracker.device = None
    tracker.batch_size = 1
    tracker.cross_hysteresis = 15
    tracker.model_person = type("M", (), {"names": {0: "person"}})()
    tracker.update_callback = None
    tracker.stream_error = ""
    return tracker


def test_crossing_near_endpoint(monkeypatch, tmp_path):
    tracker = _make_tracker(tmp_path)
    frame1 = np.zeros((100, 100, 3), dtype=np.uint8)
    frame2 = np.zeros((100, 100, 3), dtype=np.uint8)
    tracker.frame_queue.put(frame1)
    tracker.frame_queue.put(frame2)

    dets = [
        [((10, 0, 20, 20), 0.9, "person")],
        [((70, 0, 20, 20), 0.9, "person")],
    ]

    def fake_detect_batch(frames, groups):
        return [dets.pop(0) for _ in frames]

    tracker.detector = SimpleNamespace(detect_batch=fake_detect_batch)

    class StubTrack:
        def __init__(self, bbox):
            self._bbox = bbox
            self.track_id = 1
            self.det_class = "person"
            self.det_conf = 0.9
            self.age = 2
            self.det_conf = 0.9
            self.age = 2

        def is_confirmed(self):
            return True

        def to_ltrb(self):
            return self._bbox

    bboxes = [(10, 0, 30, 20), (70, 0, 90, 20)]

    class StubDS:
        def __init__(self):
            self.i = 0

        def update_tracks(self, detections, frame=None):
            bbox = bboxes[self.i]
            self.i += 1
            return [StubTrack(bbox)]

    tracker.tracker = StubDS()
    monkeypatch.setattr(manager_mod.cv2, "imwrite", lambda path, img: True, raising=False)

    inf = InferWorker(tracker)
    post = PostProcessWorker(tracker)
    inf.run()
    post.run()

    assert tracker.in_counts.get("person", 0) == 0
    assert tracker.out_counts.get("person", 0) == 1


def test_jitter_near_line_ignored(monkeypatch, tmp_path):
    tracker = _make_tracker(tmp_path)
    tracker.side_eps = 200
    frame1 = np.zeros((100, 100, 3), dtype=np.uint8)
    frame2 = np.zeros((100, 100, 3), dtype=np.uint8)
    frame3 = np.zeros((100, 100, 3), dtype=np.uint8)
    tracker.frame_queue.put(frame1)
    tracker.frame_queue.put(frame2)
    tracker.frame_queue.put(frame3)

    dets = [
        [((10, 10, 20, 20), 0.9, "person")],
        [((39, 10, 20, 20), 0.9, "person")],
        [((41, 10, 20, 20), 0.9, "person")],
    ]

    def fake_detect_batch(frames, groups):
        return [dets.pop(0) for _ in frames]

    tracker.detector = SimpleNamespace(detect_batch=fake_detect_batch)

    class StubTrack:
        def __init__(self, bbox):
            self._bbox = bbox
            self.track_id = 1
            self.det_class = "person"

        def is_confirmed(self):
            return True

        def to_ltrb(self):
            return self._bbox

    bboxes = [(10, 10, 30, 30), (39, 10, 59, 30), (41, 10, 61, 30)]

    class StubDS:
        def __init__(self):
            self.i = 0

        def update_tracks(self, detections, frame=None):
            bbox = bboxes[self.i]
            self.i += 1
            return [StubTrack(bbox)]

    tracker.tracker = StubDS()
    monkeypatch.setattr(manager_mod.cv2, "imwrite", lambda path, img: True, raising=False)

    inf = InferWorker(tracker)
    post = PostProcessWorker(tracker)
    inf.run()
    post.run()

    assert tracker.in_counts.get("person", 0) == 0
    assert tracker.out_counts.get("person", 0) == 0
