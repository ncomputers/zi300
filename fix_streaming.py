...
class StreamingFixer:
    ...
    def enable_camera_streaming(self, camera_id: int) -> bool:
        ...
        try:
            ...
            # Verify it's enabled
            time.sleep(1)
            stats_response = requests.get(f"{self.base_url}/api/cameras/{camera_id}/stats", timeout=10)
    ...
    def test_mjpeg_endpoint(self, camera_id: int) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/api/cameras/{camera_id}/mjpeg",
                timeout=10,
                stream=True
            )
    ...
    def fix_camera_configuration(self, camera_id: int) -> bool:
        ...
        try:
            ...
            print(f"🔧 Checking configuration for camera {camera_id} ({camera.get('name', 'Unknown')})...")
    ...
    def interactive_mode(self) -> None:
        ...
        for i, camera in enumerate(cameras, 1):
            ...
            print(f"  {i}. {status} {streaming} Camera {camera.get('id')} - {camera.get('name', 'Unknown')}")
        ...
        while True:
            try:
                ...
                if choice == '0':
                    ...
                    confirm = input("Fix all cameras? This may take a while (y/N): ").strip().lower()
                    if confirm == 'y':
